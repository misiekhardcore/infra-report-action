...
class ArgoCdService extends types_1.Service {
    constructor(token, config) {
        ...
        this.getResult = () => __awaiter(this, void 0, void 0, function* () {
            const { argoCd: { projects, title = this.title } } = this.config;
            if (!this.token) {
                return { messages: [], title };
            }
            const applications = (yield Promise.all(projects.map((project) => __awaiter(this, void 0, void 0, function* () { return (yield this.fetchArgoCdAplications(project)).items || []; })))).flat();
            const messages = applications.map(this.parseMessage);
            return { title, messages };
        });
        this.fetchArgoCdAplications = (project) => __awaiter(this, void 0, void 0, function* () {
            return (0, fetchUrl_1.default)(`${this.config.argoCd.url}/api/v1/applications?projects=${project}`, `Bearer ${this.token}`);
        });
        this.parseMessage = (application) => {
            return `${this.getApplicationStatus(application)} <${this.config.argoCd.url}/applications/argocd/${application.metadata.name}|${application.metadata.name}>`;
        };
...
class GithubService extends types_1.Service {
    constructor(token, config) {
        ...
        this.getWorkflowRuns = (workflows) => __awaiter(this, void 0, void 0, function* () {
            const promises = [];
        ...
        this.getLastCompletedWorkflowRuns = (workflowRuns) => {
            return workflowRuns
                .map(workflow => { var _a; return (_a = workflow.workflow_runs) === null || _a === void 0 ? void 0 : _a.find(run => run.status !== 'in_progress'); })
                .filter((run) => run !== undefined);
        };
        this.parseMessage = (run) => {
            return `${this.isRunSuccessful(run) ? '🟢' : '🔴'} <${run.html_url}|${run.name} (${run.head_branch})>`;
        };
        this.isRunSuccessful = (run) => {
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const report = yield (0, utils_1.parseReport)(services);
            core.debug(report);
    ...
    constructor(token, config) {
        ...
        this.formatResults = (projectSummary) => {
            ...
            for (const level of vulnLevels) {
                result.push(`${vulns[level]} ${(0, utils_1.capitalize)(level)}`);
            }
